...
#A dict of plugins that have been primed but not loaded
_PRIMED = {}

#A dict of all currently loaded plugins
__PLUGINS = {}
...
def prime_plugins(dirs, updatePRIMED=True, returnVar=0, verbose=True, recursive=True):
    """
    Prime a given list of packages.
    ...
    for d in dirs:
        try:
            ...
            if "dependencies" in parser['PLUGIN']:
                p.dependencies = parser['PLUGIN']['dependencies'].split(" ")
            
            if "plugins" in parser['PLUGIN'] and recursive == True:
                p.plugin_dir = parser['PLUGIN']['plugins']
                plug_dirs = get_plugins(p.plugin_dir)
                plug_primes = prime_plugins(plug_dirs, recursive=True, verbose=verbose, returnVar=0, updatePRIMED=updatePRIMED)
                p.plugins = plug_primes
...
def load_plugins2(__plugins, updatePLUGINS=True, returnVar=0, verbose=True):
    #Sort plugins by priority and load them in order
    priority_list = sorted(__plugins.items(), key=lambda k:k[1].priority)
    for p in priority_list:
        #Check dependencies are either loaded or primed for loading.
        dep_check = False
        for d in p[1].dependencies:
            if not (d in __PLUGINS or d in __plugins):
                dep_check = True
        
        if dep_check:
            print("Failed:", p[0], ", dependencies not met.")
            continue
...
def do_it_all(path):
    packages = get_plugins(path)
    _primed = prime_plugins(packages, returnVar=1)
    _plugins = load_plugins2(_primed, returnVar=1)
    return _plugins
